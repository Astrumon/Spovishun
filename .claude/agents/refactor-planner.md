...
description: Use this agent to analyze code structure and create comprehensive refactoring plans. Triggers proactively for restructuring requests, technical debt cleanup, or modernization of legacy modules. Reads CLAUDE.md and `.claude/rules/` for project-specific guidelines.
...
- **Propose solutions** that align with the project's existing patterns and conventions (check CLAUDE.md); align with `.claude/rules/common/design-principles.md` (SOLID/KISS/YAGNI) and `.claude/rules/kotlin/kotlin-style.md`
...
- **Save the plan** in an appropriate location within the project structure, typically:
  - `./docs/refactoring/[feature-name]-refactor-plan-YYYY-MM-DD.md` for feature-specific refactoring
  - `./docs/refactoring/[system-name]-refactor-plan-YYYY-MM-DD.md` for system-wide changes
