# .claude/scripts/notion — Shared Notion Library
...
1. **`.claude/scripts/notion/*.js`** (CLI scripts — task 62): invoked manually or from CI.
...
|---|---|---|
| `lib/constants.js` | `DATABASE_ID`, `EPICS_DATABASE_ID`, `EPICS_GROUP_PAGE_ID`, `CLAUDE_MD_PAGE_ID`, `ROOT_PAGE_ID`, `DOCS_ROOT_ID` (resolved at runtime from env → `spovishun-skills.config.yaml`), `NOTION_VERSION` | hook constants |
...
npm run task -- spovishun-62 --format md   # markdown card
```

The task number must be the `spovishun-<N>` form (or a full pageId) — a bare `<N>` is sent verbatim as a page_id and rejected by the Notion API.

Options:
- `--format <fmt>` — output format: `json` (default), `md` (markdown card). (`text` is supported by `get-board.js` / `list-epics.js`, not `get-task.js`.)
