---
x-spovishun: two-axis-code-review
name: two-axis-code-review
description: "Two-axis diff review against develop: Standards (CLAUDE.md + Kotlin/Clean-Architecture rules + Fowler smells) and Spec (Notion task Goal/Steps/DoD) via two parallel sub-agents with separate Ukrainian reports. Spec-vs-diff coverage only — bugs/tests/DB belong to the reviewer agents. Triggers: two-axis review, review against spec, review branch against task, spec review, review since develop, двовісне рев'ю, рев'ю по осях, звір з задачею, рев'ю проти спеки, перевір гілку проти задачі."
---
...
2. Extract the task number `N` from the `feature/spovishun-N-*` pattern.
3. Fetch the task:
   ```
...
- If the user passed a task id (`spovishun-N`, bare `N`, or a Notion pageId) as an argument — use it instead of branch parsing.
