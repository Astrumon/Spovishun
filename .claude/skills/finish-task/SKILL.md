---
x-spovishun: finish-task
name: finish-task
description: "Runs the task-completion gate — tests → build → lint (blocking) — then an advisory review that adds domain specialist skills based on what the diff touched, and closes with a plain-language summary in the project language. Never auto-merges or auto-completes. Triggers: finish task, complete task, task done, mark task complete, завершити задачу, закінчити задачу, задача готова."
---
# Finish Task
...
```
If the branch is `develop` (default `develop`) or `main`, stop: there is no
...
(e.g. `feature/spovishun-77-foo` → `feature-spovishun-77-foo_prd`).
...
while Step 5 reviews only `git diff develop...HEAD` — the **committed** diff.
...
```
(If `develop` is empty, default to `develop`.)
...
### Step 7: Plain-language summary in uk
...
> Write the summary in the project language `uk` (ISO code — `uk` is
...
Four sections, with the headings themselves written in `uk` (canonical
...
User is on `feature/spovishun-142-schedule-digest`. The diff against
`develop` is 180 substantive lines: ~120 in `domain/schedule/` (all
...
- Step 7 prints the four-section summary in `uk`, under 250 words, with
