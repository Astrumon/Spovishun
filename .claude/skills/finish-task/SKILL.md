...
name: finish-task
description: "Runs the task-completion gate — tests → build → lint (blocking) — then a code-reviewer pass on the task diff (advisory), before offering push / PR / Notion Done. Never auto-merges or auto-completes. Triggers: finish task, complete task, task done, mark task complete, завершити задачу, закінчити задачу, задача готова."
---
# Finish Task
