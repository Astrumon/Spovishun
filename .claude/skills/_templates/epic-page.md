...
<callout icon="🔬" color="green_bg">
  **Статус:** <Research завершено / Discovery / Active / On hold>.
  Гілка: `<branch from originating research task, if any>` · Задача: <spovishun-N>
</callout>
...
Order matches what `task-decomposer` will create on the board. `#N` here = position in this table, not the spovishun-N number.
...
- The `Related Notion task` URL field on the Epics DB record is for the **originating research task** (e.g. `spovishun-74`), not for "see full text here". Leave it blank if no originating task exists.
...
- Keep the body in the same primary language used in the rest of the workspace (Ukrainian by default).
