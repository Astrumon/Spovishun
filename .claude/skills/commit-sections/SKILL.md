---
x-spovishun: commit-sections
name: commit-sections
description: "Splits an already-implemented change into logical, dependency-ordered review sections and walks the user through them one at a time — stage a section, describe it in depth, hand over the commit command, verify, move on. Never commits itself. Explicit /commit-sections only."
...
> **LANGUAGE.** Talk to the user in `uk` (from `project.language`). If that
...
- Input pool = every change in the working tree relative to `HEAD` — modified, deleted, renamed
  ...
  excluding it.
- Optional argument `base-ref` (e.g. `/commit-sections develop`): when part of the
...
(`feature/spovishun-77-foo` → `.dev-context/feature-spovishun-77-foo_prd/`).
...
6. **Requirements** — the task behind the branch, resolved locally: take `N` from
   `feature/spovishun-N-slug` and read `.dev-context/{folder}/task.json` for **Goal**,
