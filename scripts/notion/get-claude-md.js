...
function readCached() {
  ...
  if (!v) return null;
  // Handle old format (plain string) migrated to new format ({content, sections})
...
async function main() {
  ...
  if (!entry) {
    ...
    // Upgrade old-format cache entry in memory (no re-write needed — TTL will expire it)
  ...
  // --format=text or --format=md: raw text (backward compatible with previous default behavior)
