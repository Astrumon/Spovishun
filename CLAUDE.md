...
- **Install / sync:** `npm install` (pulls `spovishun-skills@^1.31.0`) then
...
- **Project-owned (NOT plugin-managed), survive re-installs:**
  ...
  - `.claude/rules/common/testing.md` — **unmanaged since the 1.21.0 sync** (skipped again at 1.31.0
    with the same stderr line: `owner-authored file occupies this id`), not by choice: the
