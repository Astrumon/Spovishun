...
### Added
- Кожен рядок логу тепер несе назву чату окремою групою `[chat=…]` — у live-log видно назву,
  а не голий `chatId`. Стара регулярка клієнта лишається валідною, тож сервер релізиться
  незалежно від spovishun-admin (spovishun-194).
- `/api/v1/health` віддає версію бота і дату її релізу — spovishun-admin бачить, який білд
  живий на сервері (spovishun-194).

### Changed
- Тести Notion CLI приведені до актуальних контрактів (spovishun-192).

### Fixed
- Docker API більше не заспамлює власний лог: live-log в'юха поллила `/containers/{id}/logs`
  кожні ~3 с, і кожен полл дописував INFO-рядок у той самий лог, який читав. Успішні запити
  тепер на DEBUG, помилки лишились на WARN, тож діагностика зламаного `DOCKER_API_URL`
  збереглася (spovishun-194).
