...
class StartCommandIntegrationTest : BaseIntegrationTest() {
    @Test
    fun `start should send welcome message containing bot name`() =
        runTest {
    ...
    @Test
    fun `start should auto-register the calling user via full service stack`() =
        runTest {
            val update = buildUpdate("/start")
    ...
    @Test
    fun `start in group chat should register admins returned from telegram`() =
        runTest {
            ...
            val adminMember = com.github.kotlintelegrambot.entities
                .ChatMember(user = adminUser, status = "administrator")
    ...
    @Test
    fun `start in group chat should send registration invitation`() =
        runTest {
            val update = buildUpdate("/start", chatType = "group")
            every { bot.getChat(ChatId.fromId(testChatId)) } returns
                com.github.kotlintelegrambot.types.TelegramBotResult.Success(
    ...
    @Test
    fun `start with null message should do nothing`() =
        runTest {
            val update = com.github.kotlintelegrambot.entities
                .Update(updateId = 1L, message = null)
