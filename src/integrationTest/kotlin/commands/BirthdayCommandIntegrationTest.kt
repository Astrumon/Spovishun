...
class BirthdayCommandIntegrationTest : BaseIntegrationTest() {
    ...
    @Test
    fun `birthday set should save birthday for own user`() =
        runTest {
            registerMember()
    ...
    @Test
    fun `birthday off should clear saved birthday`() =
        runTest {
            registerMember()
    ...
    @Test
    fun `birthday should reject invalid date format`() =
        runTest {
            registerMember()
    ...
    @Test
    fun `moderator can set birthday for another registered user`() =
        runTest {
            registerMember()
    ...
    @Test
    fun `non-moderator cannot set birthday for another user`() =
        runTest {
            registerMember()
    ...
    @Test
    fun `birthday set for unregistered user returns user not registered message`() =
        runTest {
            registerMember(userId = modUserId, username = modUsername, firstName = "Mod", role = MemberRole.MODERATOR)
    ...
    @Test
    fun `markSent called twice should not throw — upsert is idempotent`() =
        runTest {
            registerMember()
            val member = memberService.getMemberByUsername(testUsername).getOrThrow()
    ...
    @Test
    fun `wasSent returns false before markSent`() =
        runTest {
            registerMember()
            val member = memberService.getMemberByUsername(testUsername).getOrThrow()
