...
class RegisterCommandIntegrationTest : BaseIntegrationTest() {
    @Test
    fun `register should save new member and send success response`() =
        runTest {
    ...
    @Test
    fun `register when already registered should send already-registered response`() =
        runTest {
            registerMember()
            val update = buildUpdate("/register")
    ...
    @Test
    fun `register with null username should store user_id fallback`() =
        runTest {
