...
class RandomCommandTest {
    ...
    fun setup() {
        clearAllMocks()
        command = RandomCommand(randomController, testMessagesProvider())
    ...
    fun `should show the group picker when no args given and the chat has groups`() = runTest {
        ...
        coEvery { randomController.groupsForPicker(chatId) } returns
        ...
        coVerify(exactly = 0) { randomController.pickRandomFromGroup(any(), any()) }
        verify {
            bot.sendMessage(
    ...
    fun `should fall back to pickRandomAll when no args given and the chat has no groups`() = runTest {
        ...
        coEvery { randomController.pickRandomAll(chatId) } returns
            CommandResponse.Success("🎲: @alice")

        command.execute(bot, update)

        coVerify { randomController.pickRandomAll(chatId) }
    ...
    fun `should call pickRandomFromGroup with lowercase key when group arg given`() = runTest {
        ...
        coEvery { randomController.pickRandomFromGroup(chatId, "devs") } returns
            CommandResponse.Success("🎲 Випало: @bob")

        command.execute(bot, update)

        coVerify { randomController.pickRandomFromGroup(chatId, "devs") }
        coVerify(exactly = 0) { randomController.pickRandomAll(any()) }
    ...
    fun `should send group not found HTML when group does not exist`() = runTest {
        ...
        coEvery { randomController.pickRandomFromGroup(chatId, "ghost") } returns
